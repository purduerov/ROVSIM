...
classdef thrustMapper %[v2.0]
    properties
        ...
        DY = 5; %[in]
        DZ = 5; %[in]
        ALPHA = deg2rad(45); %[rad]
        BETA = deg2rad(45); %[rad]
        
        thrusterCoords_VCSYS = []; %[m] In the Vehicle CSYS
        thrusterCoords = []; %[m] in the COM CSYS
        thrusterDirections = [];
    ...
    methods
        ...
        function self = thrustMapper(COM)
            ...
            self.thrusterCoords_VCSYS =  [[+dx, -dy, +dz];
                                          [+dx, +dy, +dz];
                                          [-dx, +dy, +dz];
                                          [-dx, -dy, +dz];
                                          [+dx, -dy, -dz];
                                          [+dx, +dy, -dz];
                                          [-dx, +dy, -dz];
                                          [-dx, -dy, -dz]]*0.0254; %[m] In the Vehicle CSYS
            l = ( (cos(a)/cos(b))^2 + (sin(a))^2 )^-0.5;
            t_x = l*cos(a);
            t_y = l*sin(a);
            t_z = l*cos(a)*tan(b);
            self.thrusterDirections = [[+t_x, +t_y, -t_z];
                                       [+t_x, -t_y, -t_z];
                                       [-t_x, -t_y, -t_z];
                                       [-t_x, +t_y, -t_z];
                                       [+t_x, +t_y, +t_z];
