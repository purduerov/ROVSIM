...
%  thrustMapper v3.3
%  Development Driver: Generate Thrust Envelope
%  2021.05.28

%  This script generates the independent force and moment envelopes, using
%   both the limitedMap() and the nullMap()
...
%% Discretize Domain
th = (linspace(0,360,181))'; %[deg]
phi = (linspace(-90,90,90))'; %[deg]
n_th = length(th);
n_phi = length(phi);
[TH,PHI] = meshgrid(th,phi);

%% Initialize Matrices
F_mag = 200; %[N] Bigger than the maximum possible
...
M_mag = 50; %[Nm] Bigger than the maximum possible
...
for i = 1:n_phi
    for j = 1:n_th
        ...
        thrustList2 = tm.nullMap(shiftdim(F_des(i,j,:)),[0;0;0]);
        ...
        thrustList4 = tm.nullMap([0;0;0],shiftdim(M_des(i,j,:)));
