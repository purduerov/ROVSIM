...
class ThrustMapper:
    def __init__(self):
        ...
        self.thruster_force_map = self.thruster_force_map()
    ...
    def thruster_output(self, desired_force):
        if not np.array_equal(desired_force, np.zeros(6)):
            ...
            psuedo_inv = linalg.pinv(self.thruster_force_map)
            force = np.matmul(psuedo_inv, output_needed)

            if self.fine:
                force *= 1.041  # 3.7657  # Thrust envelop inscribed circle radius
            else:
